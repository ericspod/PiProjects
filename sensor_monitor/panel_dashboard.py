...
fig_conf = dict(height=500, x_axis_type="datetime", tools="reset,xpan,xbox_zoom", sizing_mode="stretch_width")

trends = []
for c, col in zip(["temperature", "humidity", "pressure", "iaq"], COLORS):
    data = {"x": df_sel.index, "y": df_sel[c].to_numpy()}
    n = pn.indicators.Trend(label=c.title(), data=data, layout="row", height=100, plot_type="step", plot_color=col)
    trends.append(n)
